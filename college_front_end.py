...
def show_faculty_details(faculty_id):
    ...
    # Frame to hold menu and content area
    main_frame = ctk.CTkFrame(faculty_window)
    main_frame.pack(fill='both', expand=True)

    # Sidebar menu for actions
    menu_frame = ctk.CTkFrame(main_frame, width=200)
    menu_frame.pack(side='left', fill='y')

    # Label for menu title
    menu_title = ctk.CTkLabel(menu_frame, text="Actions Menu", font=("Arial", 18))
    menu_title.pack(pady=20)

    # Content frame for displaying forms based on menu selection
    content_frame = ctk.CTkFrame(main_frame)
    content_frame.pack(side='right', fill='both', expand=True)

    # Function to load specific form based on selection
    def load_form(form_function):
        # Clear previous content
        for widget in content_frame.winfo_children():
            widget.destroy()
        form_function(content_frame)

    # Menu items
    menu_items = [
        ("Update Student Marks", lambda: load_form(show_marks_update)),
        ("Update Student Attendance", lambda: load_form(show_attendance_update)),
        ("Insert Student Marks", lambda: load_form(mark_insertion)),
        ("Insert Student Attendance", lambda: load_form( attendace_insertion))
    ]

    # Add each item as a button in the menu
    for item_text, command in menu_items:
        button = ctk.CTkButton(menu_frame, text=item_text, command=command, width=180)
        button.pack(pady=10)
...
def show_insertion_forms():
    if username_entry.get()[0] == 'a':  # Check if the user is an admin
        ...
        # Button for Student Data Insertion
        insert_student_button = ctk.CTkButton(scrollable_frame, text="Register Student",
                                                command=lambda: show_student_insertion(scrollable_frame))
        insert_student_button.pack(pady=10)

        # Button for Faculty Data Insertion
        insert_faculty_button = ctk.CTkButton(scrollable_frame, text="Register Faculty",
                                                command=lambda: show_faculty_insertion(scrollable_frame))
        insert_faculty_button.pack(pady=10)
        # Button for student deletion
        delete_student_button= ctk.CTkButton(scrollable_frame, text="Remove student ",command=lambda:delete_student(scrollable_frame))
        delete_student_button.pack(pady=10)
        #Button for faculty deletion
        delete_faculty_button= ctk.CTkButton(scrollable_frame, text="Remove faculty ",command=lambda:delete_faculty(scrollable_frame))
...
#Delete student
def delete_student(scrollable_frame):
    global delete_student_id_entry
    # Clear the frame
    for widget in scrollable_frame.winfo_children():
        widget.destroy()

    # Label for the title
    ctk.CTkLabel(scrollable_frame, text="Remove Student", font=("Arial", 20)).pack(pady=20)

    # Label and entry for Student ID
    ctk.CTkLabel(scrollable_frame, text="Student ID:").pack(pady=5)
    delete_student_id_entry = ctk.CTkEntry(scrollable_frame)
    delete_student_id_entry.pack(pady=5)

    # Delete button
    delete_student_button = ctk.CTkButton(scrollable_frame, text="Remove Student", command=deletes)
    delete_student_button.pack(pady=20)

def deletes():
    # Get student ID from the entry field
    s_id = delete_student_id_entry.get()

    # Confirm the user entered an ID
    if s_id:
        # Perform deletion queries
        DB_Connect(f"DELETE FROM student WHERE s_id='{s_id}'", 0)
        DB_Connect(f"DELETE FROM attendance WHERE s_id='{s_id}'", 0)
        DB_Connect(f"DELETE FROM mark WHERE s_id='{s_id}'", 0)
        DB_Connect(f"DELETE FROM login WHERE user_id='{s_id}'",0)
        # Show confirmation message
        messagebox.showinfo("Delete Student", "Student removed successfully!")
        
        # Clear the entry field
        delete_student_id_entry.delete(0, 'end')
    else:
        messagebox.showwarning("Input Required", "Please enter a valid Student ID.")
...
def insert_stud_details():
    ...
    messagebox.showinfo("Data Insertion", "Data Inserted Successfully!")
#Delete faculty
def delete_faculty(scrollable_frame):
    global delete_faculty_id_entry
    # Clear the frame
    for widget in scrollable_frame.winfo_children():
        widget.destroy()

    # Label for the title
    ctk.CTkLabel(scrollable_frame, text="Remove Faculty", font=("Arial", 20)).pack(pady=20)

    # Label and entry for Faculty ID
    ctk.CTkLabel(scrollable_frame, text="Faculty ID:").pack(pady=5)
    delete_faculty_id_entry = ctk.CTkEntry(scrollable_frame)
    delete_faculty_id_entry.pack(pady=5)

    # Delete button
    delete_faculty_button = ctk.CTkButton(scrollable_frame, text="Delete Faculty", command=deletef)
    delete_faculty_button.pack(pady=20)

# Function to execute the deletion query
def deletef():
    # Get faculty ID from the entry field
    f_id = delete_faculty_id_entry.get()

    # Confirm the user entered an ID
    if f_id:
        # Perform deletion query
        DB_Connect(f"DELETE FROM faculty WHERE f_id='{f_id}'", 0)
        DB_Connect(f"DELETE FROM login WHERE user_id='{f_id}'",0)
        # Show confirmation message
        messagebox.showinfo("Delete Faculty", "Faculty removed successfully!")
        
        # Clear the entry field
        delete_faculty_id_entry.delete(0, 'end')
    else:
        messagebox.showwarning("Input Required", "Please enter a valid Faculty ID.")
root = ctk.CTk()
...
insert_data_button = ctk.CTkButton(root, text="Admin login", command=show_insertion_forms)
